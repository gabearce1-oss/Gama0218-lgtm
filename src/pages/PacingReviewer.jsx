...
import { Gauge, Send, Loader2, Plus, MessageSquare, Activity, Zap, Layers, TrendingUp, ScrollText, Trash2 } from 'lucide-react';
import MessageBubble from '@/components/guardrail/MessageBubble';

const QUICK_ACTIONS = [
  { icon: Gauge, label: 'Worst Pacing', prompt: 'Which chapters have the worst pacing? Rank the bottom 5 by beat_pacing and reader_retention, and tell me exactly what is wrong with each.' },
  { icon: TrendingUp, label: 'Act Momentum', prompt: 'Review all chapters in Act II. Does the act build momentum toward its climax? Identify any back-to-back slow chapters that kill momentum.' },
  { icon: Zap, label: 'Tension Sag', prompt: 'Find chapters where tension sags in the middle. Which chapters have a strong opening and closing but a weak middle? Recommend fixes.' },
  { icon: Layers, label: 'Scene Balance', prompt: 'Which chapters have the worst scene-type balance (too much narration, not enough dialogue or action)? Recommend rebalancing.' },
  { icon: ScrollText, label: 'Word Count', prompt: 'Flag chapters that are too long for their dramatic weight or too short for their stakes. Compare word_count against emotional_resonance and reader_retention.' },
  { icon: Activity, label: 'Best Pacing', prompt: 'Which 5 chapters have the best pacing? Explain what makes their beat structure and tension curves effective so I can replicate it.' },
];

const SUGGESTIONS = [
  'Review pacing for Chapter 29',
  'Compare pacing across all Act III chapters',
  'Which chapters have dead zones or rushed transitions?',
  'Find chapters where emotional payoff arrives too early',
  'Which chapters need more breath beats between action sequences?',
  'Review the pacing of the climactic chapters (38, 42, 45)',
];

export default function PacingReviewer() {
  ...
  const loadConversations = async () => {
    try {
      const convs = await base44.agents.listConversations({ agent_name: 'pacing_reviewer' });
  ...
  const handleNewConversation = async () => {
    const conv = await base44.agents.createConversation({
      ...
      metadata: { name: 'New Pacing Review', description: 'Narrative pacing analysis' }
    });
  ...
  const sendMessage = async (text) => {
    ...
    if (!activeId) {
      const conv = await base44.agents.createConversation({
        agent_name: 'pacing_reviewer',
        metadata: { name: text.slice(0, 40), description: 'Narrative pacing analysis' }
      });
      setConversations([conv, ...conversations]);
      setActiveId(conv.id);
      ...
      <div className="w-64 shrink-0 border-r border-border bg-card/50 flex flex-col">
        <div className="p-3 border-b border-border">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div>
              <div className="text-sm font-bold">Pacing Reviewer</div>
              <div className="text-[10px] text-muted-foreground uppercase tracking-wider">Narrative Structure AI</div>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {conversations.length === 0 ? (
            <p className="text-xs text-muted-foreground text-center p-4">No reviews yet.</p>
          ) : (
            conversations.map(conv => (
              ...
              >
                ...
                <span className="truncate flex-1">{conv.metadata?.name || 'Review'}</span>
                <button
        ...
        <div className="border-b border-border px-4 py-3">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <Gauge className="w-5 h-5 text-amber-400" />
              <div>
                <h1 className="text-lg font-bold">Pacing Reviewer</h1>
                <p className="text-xs text-muted-foreground">
                  Beat structure · Tension curves · Scene rhythm · Act-level momentum · Reader retention
                </p>
              </div>
            </div>
            {activeConv && (
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center max-w-2xl mx-auto">
              <Gauge className="w-12 h-12 text-amber-400/50 mb-4" />
              <h2 className="text-lg font-bold mb-2">Pacing Reviewer Ready</h2>
              <p className="text-sm text-muted-foreground mb-6">
                Ask me to review any chapter's narrative pacing — beat density, tension escalation, scene balance,
                emotional rhythm, and reader retention. I read chapter metrics and give specific, actionable structural feedback.
              </p>
        ...
        <div className="border-t border-border p-4">
          <div className="flex items-end gap-2">
            <textarea
              ...
              placeholder="Ask about a chapter's pacing, tension curve, or scene rhythm…"
              rows={1}
